...
import { Settings, Globe, User, GamepadIcon, Users, ShoppingCart } from 'lucide-react'
...
export function SettingsSidebar() {
  ...
  return (
    <aside className={`fixed left-0 top-16 bottom-0 bg-[#F3E5AB] border-r border-[#8B4513] transition-all duration-300 z-40 ${isExpanded ? 'w-64' : 'w-12 sm:w-16 md:w-20'}`}>
      <div className="h-full flex flex-col items-end p-2 sm:p-4 overflow-y-auto">
        ...
        {!isExpanded ? (
          ...
          <div className="space-y-2 sm:space-y-4 w-full">
            <div>
              <h3 className="font-semibold text-[#8B4513] mb-1 sm:mb-2 text-right text-xs sm:text-sm">{t('settings')}</h3>
              <Link href="/games-and-quizzes" className="w-full">
                <Button
                  variant="ghost"
                  className="w-full justify-end text-xs sm:text-sm"
                >
                  {t('gamesAndQuizzes')}
            ...
            <div>
              <h3 className="font-semibold text-[#8B4513] mb-1 sm:mb-2 text-right text-xs sm:text-sm">{t('language')}</h3>
              {languages.map((lang) => (
                <Button
                  key={lang.code}
                  variant="ghost"
            ...
            <div>
              <h3 className="font-semibold text-[#8B4513] mb-1 sm:mb-2 text-right text-xs sm:text-sm">{t('profile')}</h3>
              {user ? (
            ...
            <div>
              ...
              {cart.length > 0 ? (
                <div className="text-[#8B4513] text-right text-xs sm:text-sm">
                  ...
                  <Link href="/cart" className="w-full">
                    <Button variant="ghost" className="w-full justify-end mt-2 text-xs sm:text-sm">
                      {t('viewCart')}
